...
class Workflow:
    ...
    def provide(self, *patterns: str, _override=False, **kw):
        self._validated = False
        if not _override:
...
class Run:
    ...
    def resolve(self, resource_name) -> Any:
        ...
        if cell.dependencies:
            # Collapse all kw for dependecy resolution
            resolve_kw = {
                **self.wkf.base_kw,
                **route_info.kw,
                **self.extra_kw,
        ...
        bind_kw = {
            **self.wkf.base_kw,
            **route_info.typed_kw,  # We use typed version here
            **self.extra_kw,
            **config_entry,
            **dep_kw,
        }
        ...
        for alias, fn in cell.mutators.items():
            bind_kw[alias] = bind(fn, kw=bind_kw)()
